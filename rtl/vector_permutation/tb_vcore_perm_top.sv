...
module tb_vcore_perm_top;
  ...
  assign rd_rsp_valid = rd_pending_q;
  assign rd_rsp_data = vrf[rd_addr_q];
  ...
  initial begin
    ...
    check("vcompress.vm reg0 (packed actives)", vrf[4], {32'd6,32'd5,32'd3,32'd1});
    check("vcompress.vm reg1 (1 active + tail-agnostic)", vrf[5], {32'hFFFFFFFF,32'hFFFFFFFF,32'hFFFFFFFF,32'd8});
    ...
    // vcompress.vm, SEW8, LMUL8: mask bit set every 4th position (32 actives out of 128), vta=1
    begin
      logic [127:0] mask_w;
      logic [127:0] w;
      logic all_ok;
      ...
      for (int p = 0; p < 128; p++) begin
        logic [7:0] exp_e;
        logic [7:0] got_e;
        exp_e = (p < 32) ? 8'(4*p) : 8'hFF; // 32 actives packed, then vta tail
        got_e = vrf[16 + p/16][ (p%16)*8 +: 8 ];
        if (got_e !== exp_e) begin
          all_ok = 1'b0;
          fail_count++;
      ...
      if (all_ok) begin pass_count++; $display("[PASS] vcompress.vm LMUL8 (32 actives across 8 registers + tail)"); end
    end

// Mirrors vcore_alu_wb's WB_IDLE/WB_WRITE/WB_EVENT flow. The one addition
// over the ALU is a destination choice: vmv.x.s/vfmv.f.s produce a scalar
// result and go out the scalar_write port instead of the VRF write port.
module vcore_perm_wb #(
  ...
  input  vcore_perm_pkg::vcore_perm_rsp_t         result_meta_i,
  ...
  output vcore_perm_pkg::vcore_perm_commit_t      seq_ack_o
);
  import vcore_perm_pkg::*;
  typedef enum logic [1:0] {WB_IDLE, WB_WRITE, WB_EVENT} state_e;
  state_e state_q;
  logic [VLEN-1:0] data_q;
  vcore_perm_rsp_t meta_q;
  logic top_pending_q, seq_pending_q;
  logic write_fire;

  assign result_ready_o = (state_q == WB_IDLE) && !flush_i;
  assign vrf_write_valid_o    = (state_q == WB_WRITE) && !meta_q.is_scalar && !flush_i;
  assign scalar_write_valid_o = (state_q == WB_WRITE) &&  meta_q.is_scalar && !flush_i;
  assign vrf_write_data_o = data_q;
  assign vrf_write_req_o.vd_addr = meta_q.dest_addr;
  assign vrf_write_req_o.tag = meta_q.tag;
  assign scalar_write_data_o = meta_q.scalar_data;
  assign scalar_write_req_o.rd_addr = meta_q.dest_addr;
  assign scalar_write_req_o.is_fp = meta_q.is_fp;
  assign scalar_write_req_o.tag = meta_q.tag;
  assign write_fire = (vrf_write_valid_o && vrf_write_ready_i) ||
                      (scalar_write_valid_o && scalar_write_ready_i);

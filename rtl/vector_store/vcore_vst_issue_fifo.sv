// Same decoupling FIFO the permutation cluster uses, typed for the load
// decoded record: decode can retire a command while the sequencer is still
// walking the beats of the previous one.
module vcore_vst_issue_fifo #(
  ...
  input  vcore_vst_pkg::vcore_vst_decoded_t in_i,
  output logic                              out_valid_o,
  input  logic                              out_ready_i,
  output vcore_vst_pkg::vcore_vst_decoded_t out_o
);
  import vcore_vst_pkg::*;
  vcore_vst_decoded_t entries_q [DEPTH];

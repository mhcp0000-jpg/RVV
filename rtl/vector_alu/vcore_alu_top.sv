module vcore_alu_top #(
  ...
  output vcore_alu_pkg::vcore_vrf_read_req_t  vrf_read_req_o,
  input  logic                                vrf_read_rsp_valid_i,
  output logic                                vrf_read_rsp_ready_o,
  input  logic [VLEN-1:0]                     vrf_read_rsp_data_i,

  // Whole-register beat write; mask-destination comparisons write the mask reg.
  ...
  vcore_alu_decoded_t decode_data;
  logic issue_valid, issue_ready;
  vcore_alu_decoded_t issue_data;
  ...
  vcore_alu_commit_t seq_ack_data;

  vcore_alu_decode #(.VLEN(VLEN)) u_decode (
  ...
  vcore_alu_issue_fifo #(.DEPTH(ISSUE_DEPTH)) u_issue_fifo (
  ...
  vcore_alu_sequencer #(.VLEN(VLEN)) u_sequencer (
    .clk_i(clk_i), .rst_ni(rst_ni), .flush_i(flush_i),
  ...
  vcore_alu_vrf_request #(.VLEN(VLEN)) u_vrf_request (
    .clk_i(clk_i), .rst_ni(rst_ni), .flush_i(flush_i),
    ...
    .exec_ctrl_o(exec_ctrl), .exec_src1_o(exec_src1),
    .exec_src2_o(exec_src2), .exec_dst_old_o(exec_dst_old),
    .exec_mask_o(exec_mask)
  );

  vcore_alu_pipe #(.VLEN(VLEN)) u_execute (
    .clk_i(clk_i), .rst_ni(rst_ni), .flush_i(flush_i),
    .req_valid_i(exec_valid), .req_ready_o(exec_ready),
    .ctrl_i(exec_ctrl), .src1_i(exec_src1), .src2_i(exec_src2),

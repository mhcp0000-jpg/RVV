// Drives every official RVV 1.0 vector-load encoding into vcore_vld_decode
// and checks accept/reject against RVV_LOAD_CHECKLIST.csv.
//
// The point is not that the five in-scope instructions work -- tb_vcore_vld_ref
...
// first). Group 1 is all 177 encodings unmasked, group 2 is the same 177 with
// vm=0 (which turns vlm.v and the whole-register loads into reserved
// encodings), group 3 is the four scalar FP loads that share opcode 0x07, and
...
// one SEW or LMUL.
`timescale 1ns/1ps
module tb_vcore_vld_decode_table;
  ...
  initial begin
    ...
    $readmemh("out/vld_decode_inst.hex", instructions);
    $readmemh("out/vld_decode_expected.hex", expected);
    $readmemh("out/vld_decode_sew.hex", sew_vector);
    $readmemh("out/vld_decode_vlmul.hex", vlmul_vector);
    $readmemh("out/vld_decode_vl.hex", vl_vector);
    fd = $fopen("out/vld_decode_count.txt", "r");
    if (fd == 0) $fatal(1, "cannot open out/vld_decode_count.txt");
    void'($fscanf(fd, "%d", row_count));
    $fclose(fd);
    if (row_count == 0 || row_count > MAXROWS)
      $fatal(1, "bad row count %0d", row_count);

    // SEW=32, LMUL=1, vl=VLMAX: every in-scope EEW then lands on a legal EMUL
    // (8/32 = 1/4, 16/32 = 1/2, 32/32 = 1, 64/32 = 2) and vd=v8 is aligned to
    // all of them, so an encoding that is rejected here is rejected on its own
    // merits and not because of vtype.
    cmd = '0;
    cmd.vstart = 17'd0;
    cmd.vta    = 1'b1;
    cmd.vma    = 1'b1;
    ...
    for (int i = 0; i < int'(row_count); i++) begin
      ...
      if (!expected[i] && decoded.ctrl.op != VLDOP_INVALID) begin
        $display("FAIL row=%0d inst=%08h: rejected but op=%0d, not VLDOP_INVALID",
                 i, instructions[i], decoded.ctrl.op);
        fails++;
      end
    end

    $display("tb_vcore_vld_decode_table: %0d probes, %0d accepted, %0d rejected, %0d failed",
             row_count, accepted, rejected, fails);
    if (fails != 0) $fatal(1, "decode table mismatch");
    $display("tb_vcore_vld_decode_table PASS");
    $finish;
  end
endmodule
